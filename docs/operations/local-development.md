...
O script `pnpm dev` copia `.env.local` (ou `.env`) para o arquivo ignorado `apps/web/.env.local`, carrega essas variáveis e aplica as migrations versionadas antes de iniciar o Next. Se ambos existirem na raiz, `.env.local` tem precedência. Em produção, as variáveis devem ser fornecidas pelo ambiente da Coolify.
