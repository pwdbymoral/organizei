...
CI usa permissões mínimas, ações fixadas por SHA e não usa `pull_request_target`. O job `CI / e2e` gera o segredo de autenticação em runtime e usa somente PostgreSQL local. Habilite no GitHub: **Settings → Code security and analysis** → Secret scanning, Push protection e CodeQL. Em **Settings → Rules → Rulesets**, exija PR, histórico linear, bloqueio de force-push e os checks `CI / verify`, `CI / e2e`, `CodeQL / analyze` e `Security / scan`.
