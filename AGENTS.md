...
Após criar commit, push ou PR, não aguarde os checks remotos nem faça polling em loop. Registre o link e o estado inicial dos checks e encerre a tarefa, salvo se o usuário pedir acompanhamento explícito.
...
Comandos: `pnpm dev`, `pnpm db:migrate`, `pnpm test`, `pnpm e2e:setup`, `pnpm e2e:doctor`, `pnpm verify`, `pnpm verify:e2e`, `pnpm verify:all`. Para o gate completo local, use `pnpm verify:all`; o E2E oficial roda com browsers nativos preparados pelo projeto e banco efêmero isolado. A branch `main` está bloqueada para push direto; use uma branch de trabalho e o fluxo de revisão configurado. Use migrações versionadas (`generate`, `migrate`), nunca `push` em produção. Reset somente em banco não produtivo e com proteção explícita.
